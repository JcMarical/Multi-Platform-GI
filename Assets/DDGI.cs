...
public class DDGI: VolumeComponent, IPostProcessComponent
{
    ...
    [Header("Dynamic Lighting Settings动态光照设置")]
    public ClampedFloatParameter indirectIntensity = new ClampedFloatParameter(1.0f, 0.0f, 3.0f);//间接光强度

    public ClampedFloatParameter normalBiasMultiplier = new ClampedFloatParameter(0.2f, 0.0f, 1.0f);//控制法线偏差，减少自阴影问题
    
    public ClampedFloatParameter viewBiasMultiplier = new ClampedFloatParameter(0.8f, 0.0f, 1.0f);//控制视角偏差，减少视角伪影问题

    public Vector3Parameter probeRotationDegrees = new Vector3Parameter(Vector3.zero);//探针旋转
    ...
    public ClampedFloatParameter probeMinFrontfaceDistance = new ClampedFloatParameter(0.3f, 0.0f, 2.0f);//用于控制探针与物体之间的最小距离
