...
half _Cutoff;   //Alpha裁剪阈值
half _Smoothness;
half _Metallic;
half _BumpScale;// 法线贴图缩放
half _OcclusionStrength;//环境光遮蔽强度
half _Surface;//表面属性
CBUFFER_END
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    outSurfaceData.albedo               = AlphaModulate(outSurfaceData.albedo, outSurfaceData.alpha);// Alpha调制
    ...
    outSurfaceData.clearCoatMask        = half(0.0f);   //设置清漆遮罩
    outSurfaceData.clearCoatSmoothness  = half(0.0f);   //设置清漆粗糙度
}
